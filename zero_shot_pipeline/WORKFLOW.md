...
1. **Upload**: A digital pathology scanner (or user) uploads a Whole Slide Image (`.svs`) and its paired JSON metadata file to the `inputs/` directory of the Cloud Storage bucket. The metadata JSON can include an optional `"hf_model_id"` key (e.g., `"paige-ai/Virchow"`) to pick and choose the embedding model on the fly.
...
4. **Job Submission**: The Cloud Function parses the JSON for the requested `hf_model_id` (defaulting to H-optimus-0) and dynamically invokes the Vertex AI Pipelines API, passing the URIs and parameters to submit the pre-compiled `wsi_pipeline.json` graph for execution.
...
1. **WSI Download & Tiling**: The `.svs` file is downloaded to local ephemeral storage. `openslide` reads the image and systematically slices it into `224x224` pixel tiles (native resolution for the ViT models). Blank tiles (empty glass) are skipped to save compute.
2. **Open-Source Embedding (Virchow / H-optimus-0)**:
   * The pipeline dynamically downloads the requested open-source model weights via `timm` directly into the GPU-enabled KFP node.
   * Each valid tile is passed through the model (using FP16 mixed precision) to generate a dense mathematical vector (1536 dims for H-optimus, 2560 for Virchow) representing its histological features.
...
4. **Data Persistence**:
   ...
   * **Vector Search**: The open-source embedding is upserted into the **Vertex AI Vector Search Index**, allowing for future semantic similarity searches ("Find me tiles that look like this one").
...
sequenceDiagram
    ...
    participant Vision as HF Model & MedSigLIP
